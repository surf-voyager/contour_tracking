function o = replan_fitness(x_y_point,x_y_point_heading)
%%%%%%%  与函数Get_Functions_HGS表达一致%%%
%x 1-5 长度 ；6-10 航向角
global  R_path_piont R_path_piont_heading map   flag_predict%全局向量声明
...
% alpha = pi/5; %36°
alpha = 0.5419; %22°
len = size(x_y_point,2)/2;
% alpha = 0.2; %11.4592° 5m

%%%%%%%%%%%%%%%%%%%%%%%%%
% 解算‘路径点’栅格化的坐标
for c1=1:len
    [a ,~]= Rasterize_1(x_y_point(c1*2-1),x_y_point(c1*2),1,0);
    next_path_x(c1) = a(1);
    next_path_y(c1) = a(2);
    a = [];
end
...
theata = 10; % 
obj1 = (1*exp(-(min_dist-d_exp).^2./(2.*theata.^2))); %各点所占的权重
o1 = sum(obj1);
...
idx_sample_point = find(min_dist>(d_exp-1.5));
...
for c4=1:len
    if 1 == c4
        delta_theta(c4) = abs(x_y_point_heading(c4)-R_path_piont_heading); 
    else
        delta_theta(c4) = abs(x_y_point_heading(c4)-x_y_point_heading(c4-1)); 
    end
